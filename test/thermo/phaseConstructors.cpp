...
TEST(Species, fromYaml)
{
    ...
    EXPECT_EQ(S->extra.size(), (size_t) 1); // "molar-volume"
    // Check that units directive gets propagated to `extra`
    EXPECT_DOUBLE_EQ(S->extra.convert("molar-volume", "m^3/kmol"), 0.000536);
}
