...
TEST(ctonedim, freeflow)
{
    ...
    int sol = sol_newSolution("h2o2.yaml", "ohmech", "default");
    int ph = sol_thermo(sol);
    ASSERT_GE(ph, 0);
    ...
    int flow = domain_newFlow1D("free-flow", sol, "");
    ASSERT_GE(flow, 0);
...
{
    int sol = sol_newSolution("h2o2.yaml", "ohmech", "default");
    int inlet = domain_newBoundary1D("inlet", sol, "");
    ASSERT_GE(inlet, 0);
...
{
    int sol = sol_newSolution("h2o2.yaml", "ohmech", "default");
    int outlet = domain_newBoundary1D("outlet", sol, "");
    ASSERT_GE(outlet, 0);
...
{
    int interface = sol_newInterface("ptcombust.yaml", "Pt_surf", 0, 0);
    int surf = domain_newBoundary1D("reacting-surface", interface, "");
    ASSERT_GE(surf, 0);
...
{
    int sol = sol_newSolution("ptcombust.yaml", "gas", "default");
    int interface = sol_newInterface("ptcombust.yaml", "Pt_surf", 0, 0);

    // inlet and flow domains
    int inlet = domain_newBoundary1D("inlet", sol, "inlet");
    int flow = domain_newFlow1D("axisymmetric-flow", sol, "flow");
    ASSERT_EQ(flow, inlet+1);

    // reacting surface
    int reac_surf = domain_newBoundary1D("reacting-surface", interface, "surface");
    ASSERT_EQ(reac_surf, flow+1);

    // set up stack
    vector<int> doms{inlet, flow, reac_surf};
    int flame = sim1D_newSim1D(3, doms.data());
    ASSERT_GE(flame, 0);
...
TEST(ctonedim, freeflame)
{
    ...
    // flow
    int flow = domain_newFlow1D("free-flow", sol, "flow");
    ...
    // inlet
    int reac = domain_newBoundary1D("inlet", sol, "inlet");
    bdry_setMoleFractions(reac, X.c_str());
    bdry_setMdot(reac, uin * rho_in);
    bdry_setTemperature(reac, T);

    // outlet
    int prod = domain_newBoundary1D("outlet", sol, "outlet");
    ...
    for (size_t i = 0; i < nsp; i++) {
        ...
        int buflen = thermo_speciesName(ph, i, 0, 0);
        vector<char> buf(buflen);
    ...
    int comp = domain_componentIndex(flow, "T");
    ASSERT_EQ(comp, 2);
