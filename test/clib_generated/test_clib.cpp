...
string reportError()
{
    ...
    int buflen = ct_getCanteraError(0, output_buf.data());
...
TEST(ct, cabinet_exceptions)
{
    ...
    int ref = sol_newSolution("h2o2.yaml", "ohmech", "default");
    sol_del(ref);
    int thermo = sol_thermo(ref);
    EXPECT_EQ(thermo, -2);
    err = reportError();
...
TEST(ct, new_solution)
{
    ...
    string name = "ohmech";
    int ref = sol_newSolution("h2o2.yaml", name.c_str(), "default");
    ASSERT_EQ(ref, 0);
    ...
    int buflen = sol_name(ref, 0, 0); // includes \0
...
TEST(ct, sol_objects)
{
    ...
    int ref = sol_newSolution("gri30.yaml", "gri30", "none");
    ASSERT_EQ(ref, 0);
    ASSERT_EQ(thermo_cabinetSize(), 1); // one ThermoPhase object

    int ref2 = sol_newSolution("h2o2.yaml", "ohmech", "default");
    ASSERT_EQ(ref2, 1);
...
TEST(ct, new_interface)
{
    ...
    int sol = sol_newSolution("ptcombust.yaml", "gas", "none");
    ASSERT_EQ(sol, 0);

    vector<int> adj{sol};
    int surf = sol_newInterface("ptcombust.yaml", "Pt_surf", 1, adj.data());
    ASSERT_EQ(surf, 1);

    int ph_surf = sol_thermo(surf);
    int buflen = sol_name(ph_surf, 0, 0) + 1; // include \0
    vector<char> buf(buflen);
    sol_name(ph_surf, buflen, buf.data());
    string solName(buf.data());
    ASSERT_EQ(solName, "Pt_surf");

    int kin_surf = sol_kinetics(surf);
...
TEST(ct, new_interface_auto)
{
    ...
    vector<int> adj;
    int surf = sol_newInterface("ptcombust.yaml", "Pt_surf", 0, adj.data());
    ASSERT_EQ(surf, 0);

    ASSERT_EQ(sol_nAdjacent(surf), 1u);
    int gas = sol_adjacent(surf, 0);
    ASSERT_EQ(gas, 1);

    int buflen = sol_name(gas, 0, 0) + 1; // include \0
    vector<char> buf(buflen);
...
TEST(ct, transport)
{
    int sol0 = sol_newSolution("gri30.yaml", "gri30", "default");
    int thermo = sol_thermo(sol0);
    int tran = sol_transport(sol0);
