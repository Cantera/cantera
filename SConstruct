...
config_options = [
    ...
    EnumOption(
        "julia_docs",
        """Generate the API reference pages for the Julia interface as part of the
           Sphinx documentation. This requires a Julia installation. If set to
           'default', the pages are generated if the 'julia' command is found on the
           'PATH', and skipped with a placeholder page otherwise. Set this to 'y' to
           treat a missing Julia installation as an error.""",
        "default", ("n", "y", "default")),
    BoolOption(
        "example_data",
...
if env['sphinx_docs']:
    ...
    # The Julia API reference is generated by running the Julia interface, so it can
    # only be built where Julia is installed. Local doc builds tolerate its absence.
    if env['julia_docs'] != 'n':
        if which('julia'):
            env['julia_docs'] = 'y'
        elif env['julia_docs'] == 'y':
            config_error("'julia_docs' was set to 'y', but the 'julia' command was "
                         "not found on the 'PATH'.")
        else:
            env['julia_docs'] = 'n'
            logger.info("The 'julia' command was not found; the API reference for "
                        "the Julia interface will not be generated.")
