...
namespace Cantera {
    ...
    void IdealGasThermo::getChemPotentials(doublereal* mu) const {
        doublereal logp = log(pressure()/m_spthermo->refPressure());
        doublereal xx;
        doublereal rt = m_s->temperature() * GasConstant;
        const array_fp& g_RT = gibbs_RT();
        for (int k = 0; k < m_kk; k++) {
            xx = fmaxx(SmallNumber, m_s->moleFraction(k));
    ...
    void IdealGasThermo::initThermo(Phase& s) {
        Thermo::initThermo(s);
        m_kk = s.nSpecies();
        m_mm = s.nElements();
        ...
        // allocate space to cache species thermo properties
        m_kk = m_s->nSpecies();
     ...
     *
     */
    void IdealGasThermo::setToEquilState(const doublereal* lambda_RT) 
    {
    ...
    void IdealGasThermo::_updateThermo() const {
        doublereal tnow = m_s->temperature();
        if (m_tlast != tnow) {
