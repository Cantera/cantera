...
namespace VCSnonideal {
  ...
  static int setUpElements(vcs_VolPhase *VolPhase, ThermoPhase *tPhase) {
    ...
    if (cne) {
      VolPhase->ChargeNeutralityElement = ne;
      ne++;
    }
	...
	for (eT = 0; eT < nebase; eT++) {
	  ...
	  if (ename == "E") {
	    eFound = eT;
	    VolPhase->ElActive[eT] = 0;
	    VolPhase->m_elType[eT] = VCS_ELEM_TYPE_ELECTRONCHARGE;
	  }
	}
      } else {
	for (eT = 0; eT < nebase; eT++) {
      ...
      if (eFound == -2) {
	eFound = ne;
	VolPhase->m_elType[ne] = VCS_ELEM_TYPE_ELECTRONCHARGE;
	VolPhase->ElActive[ne] = 0;
	string ename = "E";
    ...
    if (cne) {
      ...
      if (pname == "") {
	char sss[50];
	sprintf(sss, "phase%d", VolPhase->VP_ID);
	pname = sss;
      }
      ename = "cn_" + pname;
      e = VolPhase->ChargeNeutralityElement;
      VolPhase->ElName[e] = ename;
    }
    ...
    if (cne) {
      for (k = 0; k < ns; k++) {
	fm[VolPhase->ChargeNeutralityElement][k] = tPhase->charge(k);
      }
    }
