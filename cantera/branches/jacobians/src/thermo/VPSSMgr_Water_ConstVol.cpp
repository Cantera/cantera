...
VPSSMgr_Water_ConstVol::VPSSMgr_Water_ConstVol(VPStandardStateTP* vp_ptr, SpeciesThermo<doublereal> * spth) :
        VPSSMgr(vp_ptr, spth),
        m_waterSS(0)
{
...
void VPSSMgr_Water_ConstVol::initAllPtrs(VPStandardStateTP* vp_ptr, SpeciesThermo* sp_ptr)
{
    ...
    if (!m_waterSS) {
        throw CanteraError("VPSSMgr_Water_ConstVol::initAllPtrs", "bad dynamic cast");
    }
}
...
void VPSSMgr_Water_ConstVol::initThermoXML(XML_Node& phaseNode, const std::string& id)
{
    ...
    if (!m_waterSS) {
        throw CanteraError("VPSSMgr_Water_ConstVol::initThermoXML", "bad dynamic cast");
    }
    ...
    for (size_t k = 1; k < m_kk; k++) {
        ...
        if (!s) {
            throw CanteraError("VPSSMgr_Water_ConstVol::initThermoXML", "no species Node for species " + sss[k]);
        }
        const XML_Node* ss = s->findByName("standardState");
        if (!ss) {
            std::string sName = s->operator[]("name");
            throw CanteraError("VPSSMgr_Water_ConstVol::initThermoXML", "no standardState Node for species " + sName);
        }
        std::string model = (*ss)["model"];
        if (model != "constant_incompressible") {
            std::string sName = s->operator[]("name");
            throw CanteraError("VPSSMgr_Water_ConstVol::initThermoXML", "standardState model for species isn't "
...
PDSS*
VPSSMgr_Water_ConstVol::createInstallPDSS(size_t k, const XML_Node& speciesNode, const XML_Node* const phaseNode_ptr)
{
    ...
    if (k == 0) {
        ...
        if (xn != "H2O(L)") {
            throw CanteraError("VPSSMgr_Water_ConstVol::installSpecies", "h2o wrong name: " + xn);
        }
        ...
        const XML_Node* ss = speciesNode.findByName("standardState");
        if (!ss) {
            throw CanteraError("VPSSMgr_Water_ConstVol::installSpecies", "no standardState Node for species " + speciesNode.name());
        }
        std::string model = (*ss)["model"];
        if (model != "constant_incompressible") {
            throw CanteraError("VPSSMgr_Water_ConstVol::initThermoXML", "standardState model for species isn't "
