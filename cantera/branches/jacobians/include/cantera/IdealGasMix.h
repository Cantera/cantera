...
class IdealGasMix: public IdealGasPhase<doublereal>, public GasKinetics
{
public:
            ...
            m_r(0)
    {
        ...
        if (!m_ok)
            throw CanteraError("IdealGasMix", "Cantera::buildSolutionFromXML returned false");
    }

    IdealGasMix(XML_Node& root, std::string id_) :
            m_ok(false),
            m_r(&root),
            m_id(id_)
    {
    ...
    IdealGasMix(const IdealGasMix& other) :
            m_ok(false),
            m_r(other.m_r),
            m_id(other.m_id)
    {
