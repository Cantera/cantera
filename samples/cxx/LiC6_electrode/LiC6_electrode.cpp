...
void calc_potentials()
{
    ...
    ThermoPhase* electrodebulk = newPhase(filename,phasename);
    std::string intercalatingSpeciesName("Li(C6)");
    int intercalatingSpeciesIdx = electrodebulk->speciesIndex(intercalatingSpeciesName);
    size_t nsp_tot = electrodebulk->nSpecies();

    std::ofstream fout;
    fout.open("potentials_output.dat", std::ofstream::out);
    ...
    double* spvals = new double [nsp_tot];
    double* actCoeff = new double [nsp_tot];
    double* dlnActCoeffdlnX_diag = new double [nsp_tot];
    double xmin = 0.6;
    double xmax = 0.9;
	...
	if (j != intercalatingSpeciesIdx){
      ...
      fout << xv[0] << " " << spvals[0] << " " << spvals[1] << " " << Uref
	   << " " << actCoeff[0] << " " << actCoeff[1]
	   << " " << dlnActCoeffdlnX_diag[0] << " " << dlnActCoeffdlnX_diag[1] << std::endl;
    }

    fout.close();

    if(spvals) {delete[] spvals; spvals = NULL;}
    if(actCoeff) {delete[] spvals; spvals = NULL;}
    if(dlnActCoeffdlnX_diag) {delete[] spvals; spvals = NULL;}
