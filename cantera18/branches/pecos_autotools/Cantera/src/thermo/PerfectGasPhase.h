...
#ifndef CT_PERFECTGASPHASE_H
#define CT_PERFECTGASPHASE_H
...
namespace Cantera {
  ...
  //!  Class %PerfectGasPhase represents low-density gases that obey the
  //!  ideal gas equation of state. 
  /*!
   *
   * %PerfectGasPhase derives from class ThermoPhase,
   ...
   *  at arbitray pressure are provided in %PerfectGasPhase. However, they
   ...
   * for %Cantera. A new %PerfectGasPhase may be created by the following code
   * snippet:
   *
   * @code
   ...
   *    PerfectGasPhase *silaneGas = dynamic_cast <PerfectGasPhase *>(silane_tp);
   * @endcode
   *
   ...
   *    XML_Node *xc = get_XML_File("silane.xml"); 
   *    XML_Node * const xs = xc->findNameID("phase", "silane");
   *    PerfectGasPhase *silaneGas = new PerfectGasPhase(*xs);
   * @endcode
   *
   * <HR>
   * <H2> XML Example </H2>
   * <HR>
   *   An example of an XML Element named phase setting up a PerfectGasPhase
   ...
   *   being of the type handled by the PerfectGasPhase object. 
   *
   *    @ingroup thermoprops
   *
   */
  class PerfectGasPhase : public ThermoPhase  {
  ...
  public:
     ...
     */
    PerfectGasPhase(const PerfectGasPhase &right);
     ...
     * @param right Object to be copied.
     */
    PerfectGasPhase& operator=(const  PerfectGasPhase& right);

    //! Destructor
    virtual ~PerfectGasPhase() {}
     ...
     */
    virtual int eosType() const { return cPerfectGas; }
