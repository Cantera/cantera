...
#ifndef CT_PECOSGASPHASE_H
#define CT_PECOSGASPHASE_H
...
namespace Cantera {
  ...
  //!  Class %PecosGasPhase represents low-density gases that obey the
  //!  ideal gas equation of state. 
  /*!
   *
   * %PecosGasPhase derives from class ThermoPhase,
   ...
   * for %Cantera. A new %PecosGasPhase may be created by the following code
   * snippet:
   *
   * @code
   ...
   *    PecosGasPhase *silaneGas = dynamic_cast <PecosGasPhase *>(silane_tp);
   * @endcode
   *
   ...
   *    XML_Node *xc = get_XML_File("silane.xml"); 
   *    XML_Node * const xs = xc->findNameID("phase", "silane");
   *    PecosGasPhase *silaneGas = new PecosGasPhase(*xs);
   * @endcode
   *
   * <HR>
   * <H2> XML Example </H2>
   * <HR>
   *   An example of an XML Element named phase setting up a PecosGasPhase
   ...
   *   being of the type handled by the PecosGasPhase object. 
   *
   *    @ingroup thermoprops
   *
   */
  class PecosGasPhase : public ThermoPhase  {
  ...
  public:
     ...
     */
    PecosGasPhase(const PecosGasPhase &right);
     ...
     * @param right Object to be copied.
     */
    PecosGasPhase& operator=(const  PecosGasPhase& right);

    //! Destructor
    virtual ~PecosGasPhase() {}
    ...
    /*!
     *  Returns the value cPecosGas, defined  in mix_defs.h.
     */
    virtual int eosType() const { return cPecosGas; }
