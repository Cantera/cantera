...
Compilation of the auto-generated CLib interface is fully integrated into the build
process. The API is generated by building Cantera with the `clib_experimental=y` option:

```bash
scons build clib_experimental=y
```

The CLib test suite is invoked by running:
...
Auto-generated files are placed in the output folder `interfaces/clib`, which is the
