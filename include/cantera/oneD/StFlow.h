...
// CERFACS : This file is no more used in CANTERA. CERFACS devs done in this file should be moved somewhere else (Flow1D or in a specific cantera-avbp file)
...
public:
    ...
    StFlow(ThermoPhase* ph = 0, size_t nsp = 1, size_t nsoot = 0, size_t neq = c_offset_Y, size_t points = 1);

    //! Delegating constructor
    StFlow(shared_ptr<ThermoPhase> th, size_t nsp = 1, size_t nsoot = 0, size_t neq = c_offset_Y, size_t points = 1);
    ...
    //! @param points  initial number of grid points
    StFlow(shared_ptr<Solution> sol, const string& id="", size_t nsoot = 0, size_t neq = c_offset_Y, size_t points=1);

    virtual ~StFlow(); // virtual kw required for derived class destructors (flamelet)
    ...
    void setFreeFlow() {
        // m_type = cFreeFlow; // AP : test to remove the domain type (not used in Flow1D.h)
        m_dovisc = false;
        m_isFree = true;
        m_usesLambda = false;
    }
    ...
    void setAxisymmetricFlow() {
        // m_type = cAxisymmetricStagnationFlow; // AP : test to remove the domain type (not used in Flow1D.h)
        m_dovisc = true;
        m_isFree = false;
        m_usesLambda = true;
    }
    ...
    void setFlameletFlow() {
        // m_type = cFlameletFlow; // AP : test to remove the domain type (not used in Flow1D.h)
        m_dovisc = true;
        m_isFree = false;
        m_usesLambda = true;
    }
