...
//! ```
class DomainFactory : public Factory<Domain1D, shared_ptr<Solution>, const string&, const size_t&, const size_t&>
{
public:
    /**
...
shared_ptr<T> newDomain(
    const string& domainType, shared_ptr<Solution> solution, const string& id="", const size_t& sections=0, const size_t& fictives=0)
{
    string id_ = id;
    if (id_ == "") {
        id_ = domainType;
    ...
    auto ret = std::dynamic_pointer_cast<T>(
        shared_ptr<Domain1D>(
            DomainFactory::factory()->create(domainType, solution, id_, sections, fictives)));
    if (!ret) {
