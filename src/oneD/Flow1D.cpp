...
void Flow1D::resize(size_t ncomponents, size_t points)
{
    ...
    // Soot
    if (m_nsoot > 0){
        sootConsumption.resize(m_nsp, m_points, 0.0);
...
void Flow1D::setupGrid(size_t n)
{
    resize(m_nv, n);
...
void Flow1D::calcMixFrac(const double* x, size_t j) {
                  ...
                  integer* diagGlobal, double rdt)
{
    ...
    // CERFACS : Do not change the eval order : Soot -> Species -> Energy
                        ...
                        double rdt, size_t jmin, size_t jmax)
{
    ...
    for (size_t j = j0; j <= j1; j++) {
        if (m_do_energy[j]) {
            ...
            double sum2 = 0.0;
            for (size_t k = 0; k < m_nsp; k++) {
                double flxk = 0.5*(m_flux(k, j-1)*avbp_thick[j-1] + m_flux(k, j)*avbp_thick[j]);
                         ...
                         double rdt, size_t jmin, size_t jmax)
{
    if (jmin == 0) { // left boundary
        ...
        for (size_t k = 0; k < m_nsp; k++) {
            sum += Y(x,k,jmin);
            rsd[index(c_offset_Y+k, jmin)] = -(m_flux(k, jmin) * avbp_thick[jmin] +
    ...
    if (jmax == m_points - 1) { // right boundary
        ...
        if (m_nsoot > 0){
            for (size_t k = 0; k < m_nsoot; k++){
                if (m_do_retroaction){sum += Ys(x,k,jmax);} // CERFACS : Done for right Bnd but not for left ?? Maybe not needed because there are no soot at the inlet ?
            }
        }
        for (size_t k = 0; k < m_nsp; k++) {
            sum += Y(x,k,jmax);
            rsd[index(k+c_offset_Y, jmax)] = m_flux(k, jmax-1) * avbp_thick[jmax-1] +
        ...
        if (jmin == 0) { // left boundary
            for (size_t k = 0; k < m_nsoot; k++){
                rsd[index(c_offset_S + k, jmin)] = - (m_soot_diff(k,jmin)
                                                + rho_u(x,jmin)* Ys(x,k,jmin));
                diag[index(c_offset_S + k, jmin)] = 0;
            }
        }
        if (jmax == m_points - 1) { // right boundary
            for (size_t k = 0; k < m_nsoot; k++){ //right boundary
                rsd[index(c_offset_S + k, jmax)] =
                    m_soot_diff(k,jmax-1) + rho_u(x,jmax)*Ys(x,k,jmax);
                diag[index(c_offset_S + k, jmax)] = 0;
            }
        }
        // j0 and j1 are constrained to only interior points
        size_t j0 = std::max<size_t>(jmin, 1);
...
void Flow1D::setPrecursors(vector<size_t> id_precursors){
    //----------------------------------------------------
...
void Flow1D::sootCollisionInverse(){
    //----------------------------------------------------
...
void Flow1D::sootSource(const double* x, size_t j) {
    //----------------------------------------------------
...
void Flow1D::sootDimerization(const double* x, size_t j){
  //----------------------------------------------------
  ...
  //----------------------------------------------------
    ...
    for (size_t k = 0; k < n_PAH; k++){
        ...
        sootConsumption(m_precursors[k],j) = 2 * omega_k;
        // Adds PAH contribution to overall local dimerization rate sootConsumption[index(k,j)]
        omega += omega_k;
...
void Flow1D::sootNucleation(const double* x, size_t j){
    //----------------------------------------------------
...
void Flow1D::sootCondensation(const double* x, size_t j){
    //----------------------------------------------------
...
void Flow1D::sootCoagulation(const double* x, size_t j){
    //----------------------------------------------------
...
void Flow1D::updateSootDiffFluxes(const double* x, size_t j0, size_t j1)
{
    //----------------------------------------------------
...
void Flow1D::sootSurface(const double* x, size_t j){
    //----------------------------------------------------
...
void Flow1D::sootSurfaceInitialization(const double* x, size_t j){
    ...
    if (m_do_retroaction){
        ...
        if (m_haca_model == 1){
            k = m_thermo->speciesIndex("H");
            if (k != npos){sootConsumption(k,j) = - (w01f - w01b) - (w03f - w03b) + (w05f - w05b);}
            k = m_thermo->speciesIndex("C2H2");
            if (k != npos){sootConsumption(k,j) = (w04f - w04b);}
            k = m_thermo->speciesIndex("H2");
            if (k != npos){sootConsumption(k,j) =  (w01f - w01b);}
            k = m_thermo->speciesIndex("O2");
            if (k != npos){sootConsumption(k,j) = - w06f - w06bisf;}
            k = m_thermo->speciesIndex("CO");
            if (k != npos){sootConsumption(k,j) = 2.0 * w06f;}
            k = m_thermo->speciesIndex("OH");
            if (k != npos){sootConsumption(k,j) = - (w02f - w02b) - w07f;}
            k = m_thermo->speciesIndex("H2O");
            if (k != npos){sootConsumption(k,j) = + (w02f - w02b);}
            k = m_thermo->speciesIndex("HCO");
            if (k != npos){sootConsumption(k,j) = 2.0 * w06bisf + w07f;}
            k = m_thermo->speciesIndex("CH");
            if (k != npos){sootConsumption(k,j) = w07f;}
        }else if (m_haca_model == 2){
            k = m_thermo->speciesIndex("H");
            if (k != npos){sootConsumption(k,j) = -(w01f-w01b) + (w03f-w03b) + w04f;}
            k = m_thermo->speciesIndex("C2H2");
            if (k != npos){sootConsumption(k,j) = -w04f;}
            k = m_thermo->speciesIndex("H2");
            if (k != npos){sootConsumption(k,j) =  (w01f-w01b) + w05f + w06f;}
            k = m_thermo->speciesIndex("O2");
            if (k != npos){sootConsumption(k,j) = -w05f;}
            k = m_thermo->speciesIndex("CO");
            if (k != npos){sootConsumption(k,j) = 2.0 * w05f + w06f;}
            k = m_thermo->speciesIndex("OH");
            if (k != npos){sootConsumption(k,j) = -(w02f-w02b) - w06f;}
            k = m_thermo->speciesIndex("H2O");
            if (k != npos){sootConsumption(k,j) = (w02f-w02b);}        
        }else if (m_haca_model == 3){
            k = m_thermo->speciesIndex("H");
            if (k != npos){sootConsumption(k,j) = -(w01f-w01b) - w02f + w03f;}
            k = m_thermo->speciesIndex("C2H2");
            if (k != npos){sootConsumption(k,j) = -w03f;}
            k = m_thermo->speciesIndex("H2");
            if (k != npos){sootConsumption(k,j) =  (w01f-w01b) + w05f;}
            k = m_thermo->speciesIndex("O2");
            if (k != npos){sootConsumption(k,j) = -w04f;}
            k = m_thermo->speciesIndex("CO");
            if (k != npos){sootConsumption(k,j) = 2.0 * w04f + w05f;}
            k = m_thermo->speciesIndex("OH");
            if (k != npos){sootConsumption(k,j) = - w05f;}
        }
    }
...
void Flow1D::getDistributionOrdre0(const double* x, size_t j){
    for (size_t k = 0; k < m_nsoot; k++){
