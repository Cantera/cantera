...
double Electron::electronDiffusivity(double N)
{
    ...
    for (size_t j = 0; j < m_points - 1; j++) {
        ...
        double right = 0.0;
        if (m_eps[j] != 0.0) {
            left = m_eps[j] * m_f0[j] / (m_totalCrossSection[j] + vi / pow(m_eps[j], 0.5));
        }
        if (m_eps[j+1] != 0.0) {
            right = m_eps[j+1] * m_f0[j+1] / (m_totalCrossSection[j+1] + vi / pow(m_eps[j+1], 0.5));
        }
        sum += 0.5 * (left + right) * (m_eps[j+1] - m_eps[j]);
    }
...
double Electron::electronMobility(double N)
{
    ...
    for (size_t j = 0; j < m_points - 1; j++) {
        ...
        double right = 0.0;
        if (m_eps[j] != 0.0) {
            left = m_eps[j] * m_df0[j] / (m_totalCrossSection[j] + vi / pow(m_eps[j], 0.5));
        }
        if (m_eps[j+1] != 0.0) {
            right = m_eps[j+1] * m_df0[j+1] / (m_totalCrossSection[j+1] + vi / pow(m_eps[j+1], 0.5));
        }
        sum += 0.5 * (left + right) * (m_eps[j+1] - m_eps[j]);
    }
    return -1./3. * m_gamma * sum / N;
