...
!!$  end function newTransport
  ...
  double precision function ctrans_electronThermalConductivity(self)
    implicit none
    type(phase_t), intent(inout) :: self
    ctrans_electronThermalConductivity = trans_electronThermalConductivity(self%tran_id)
    self%err = 0
  end function ctrans_electronThermalConductivity

  subroutine ctrans_getMobilities(self, d)
    implicit none
    type(phase_t), intent(inout) :: self
    double precision, intent(out) :: d(*)
    self%err = trans_getMobilities(self%tran_id, d)
  end subroutine ctrans_getMobilities
