...
bool RedlichKwongMFTP::addSpecies(shared_ptr<Species> spec)
{
    ...
    if (added) {
        ...
        // Initialize a_vec and b_vec to -1, to screen for species with pureFluidParameters which
...
void RedlichKwongMFTP::initThermoXML(XML_Node& phaseNode, const std::string& id)
{
    if (phaseNode.hasChild("thermo")) {
        ...
        // Go get all of the coefficients and factors in the activityCoefficients XML block
        if (thermoNode.hasChild("activityCoefficients")) {
            XML_Node& acNode = thermoNode.child("activityCoefficients");

            // Count the number of species with parameters provided in the input file:
            size_t nParams = 0;

            // Loop through the children and read out fluid parameters.  Process all the
            // pureFluidParameters, first:
            ...
            // If any species exist which have undefined pureFluidParameters, search the database
            //     in 'critProperties.xml' to find critical temperature and pressure and calculate
            //     a and b based on that.
            if (m_kk > nParams) // activity coefficients of all species are not listed in CTI file. Look into the database.
            {
                // Get the array of species names:
                XML_Node& sparrays = phaseNode.child("speciesArray");
                std::vector<std::string> spnames;
                getStringArray(sparrays, spnames);
                ...
                for (size_t i = 0; i < m_kk; i++)
                {
                    string iName = ba::to_lower_copy(spnames[i]);
                    ...
                    if (a_coeff_vec(0, counter) == -1 ||
                        ...
                        doublereal *coeffArray;

                        // Search the database for the species name and calculate coefficients
                        // a and b, from critical properties:
                        // coeffArray[0] = a0, coeffArray[1] = b;
                        coeffArray = getCoeff(iName);

                        // Check if species was found in the database of critical properties,
                        // and either assign the results or throw an error:
                        if (coeffArray[0] != -1 || coeffArray[1] != -1) {
                            //Assuming no temperature dependence (i,e a1 = 0)
                            a_coeff_vec(0, counter) = coeffArray[0];
                            a_coeff_vec(1, counter) = 0.0;
                            b_vec_Curr_[iSpecies] = coeffArray[1];
                            
                            setSpeciesCoeffs(iName, a_coeff_vec(0, counter),
                                             a_coeff_vec(1, counter),
...
doublereal* RedlichKwongMFTP::getCoeff(const std::string& iName)
{
    ...
    // Get number of species in the databae
    // open xml file critProperties.xml
    XML_Node* doc = get_XML_File("critProperties.xml");
    size_t nDatabase = doc->nChildren();
    XML_Node* acNodeDocPtr = 0;

    // Loop through all species in the database and attempt to match supplied species to each.
    // If present, calculate pureFluidParameters a_k and b_k based on crit properties T_c and P_c:
    for (size_t isp = 0; isp < nDatabase; isp++)
    {
        XML_Node& acNodeDoc = doc->child(isp);
        acNodeDocPtr = &acNodeDoc;
        std::string dbName = ba::to_lower_copy(acNodeDoc.attrib("name"));

        // Attempt to match provided specie iName to current database specie dbName:
        if (iName == dbName)
        {
            // Read from database and calculate a and b coefficients
            doublereal vParams;
            static doublereal T_crit, P_crit;
            ...
            //Pc from tha database is in bars. convert to Pascals
