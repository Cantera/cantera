...
void RedlichKwongMFTP::initThermo()
{
    for (auto& item : m_species) {
        // Read a and b coefficients from species 'extra' information (i.e. as
        // specified in a YAML input file)
        if (item.second->extra.hasKey("Redlich-Kwong")) {
            AnyMap& rk = item.second->extra["Redlich-Kwong"].as<AnyMap>();
            double a0 = 0, a1 = 0;
            if (rk["a"].isScalar()) {
                a0 = rk.convert("a", "Pa*m^6/kmol^2*K^0.5");
            } else {
                auto avec = rk["a"].asVector<AnyValue>(2);
                a0 = rk.units().convert(avec[0], "Pa*m^6/kmol^2*K^0.5");
                a1 = rk.units().convert(avec[1], "Pa*m^6/kmol^2/K^0.5");
            }
            double b = rk.convert("b", "m^3/kmol");
            setSpeciesCoeffs(item.first, a0, a1, b);
            if (rk.hasKey("binary-a")) {
                AnyMap& binary_a = rk["binary-a"].as<AnyMap>();
