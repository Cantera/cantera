...
shared_ptr<Species> newSpecies(const XML_Node& species_node)
{
    ...
    if (species_node.hasChild("stoichIsMods")) {
        s->extra["weak-acid-charge"] = getFloat(species_node, "stoichIsMods");
    }

    if (species_node.hasChild("electrolyteSpeciesType")) {
        s->extra["electrolyte-species-type"] = species_node.child("electrolyteSpeciesType").value();
    }
    ...
    if (thermo && thermo->attrib("model") == "IonFromNeutral") {
        if (thermo->hasChild("specialSpecies")) {
            s->extra["standard-state"]["special-species"] = true;
        }
    }
