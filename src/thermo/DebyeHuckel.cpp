...
void DebyeHuckel::initThermo()
{
    ...
    if (m_extra.hasKey("activity-data")) {
        auto& node = m_extra["activity-data"].as<AnyMap>();
...
bool DebyeHuckel::addSpecies(shared_ptr<Species> spec)
{
    ...
    if (added) {
        ...
        m_Aionic.push_back(spec->extra.convert("ionic-radius", "m", NAN));
        ...
        if (spec->extra.hasKey("weak-acid-charge")) {
            stoichCharge = spec->extra["weak-acid-charge"].asDouble();
        ...
        if (spec->extra.hasKey("electrolyte-species-type")) {
            est = interp_est(spec->extra["electrolyte-species-type"].asString());
        }
