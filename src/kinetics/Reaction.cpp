                          ...
                          const Kinetics& kin)
{
    ...
    if (node["type"].asString() == "falloff") {
        R.low_rate = readArrhenius(R, node["low-rate"], kin, node.units(), 1);
        R.high_rate = readArrhenius(R, node["high-rate"], kin, node.units());
    } else { // type == "chemically-activated"
        R.low_rate = readArrhenius(R, node["low-rate"], kin, node.units());
        R.high_rate = readArrhenius(R, node["high-rate"], kin, node.units(), -1);
    }
...
void setupPlogReaction(PlogReaction& R, const AnyMap& node, const Kinetics& kin)
{
    setupReaction(R, node);
    std::multimap<double, Arrhenius> rates;
    for (const auto& rate : node["rates"].asVector<AnyMap>()) {
                            ...
                            const Kinetics& kin)
{
    ...
    if (node.hasKey("rate")) {
        ...
        throw InputFileError("setupInterfaceReaction", node,
            "Reaction must include either a 'rate' or 'sticking-coefficient' "
            "node.");
    }
