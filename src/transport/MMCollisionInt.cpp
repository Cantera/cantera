...
TransportReadExt heavy;
C_MMomega11 = heavy.C_omega11;
C_MMomega22 = heavy.C_omega22;
C_MMbstar = heavy.C_bstar;
PairsMM = heavy.Pairs;
...
void MMCollisionInt::replaceMinus(string &species1, string &species2)
{
// the negative sign is used in the external files to separate collisional pairs.
// for this reason, negative ions are intorduced as Xm

replace( species1.begin(), species1.end(), '-', 'm' );
replace( species2.begin(), species2.end(), '-', 'm' );

}



void MMCollisionInt::getCoefficients_omega11(string s1, string s2, vector<double> &coefficients, double T)
{

replaceMinus(s1, s2);

for (size_t i=0; i<PairsMM.size()/2; i++ )

{

        if (2*i != PairsMM.size())
        {
                if ( ( (PairsMM[2*i] == s1) and (PairsMM[2*i+1] == s2) ) or ( (PairsMM[2*i] == s2) and (PairsMM[2*i+1] == s1)) )

                {
            		for (size_t j=0; j<4; j++)
            		{
                		coefficients.push_back(C_MMomega11[4*i+j]);
        		}
                }
        ...
        else { cout << "coefficients not found for " << s1 << " and " << s2 << endl; }
...
void MMCollisionInt::getCoefficients_omega22(string s1, string s2, vector<double> &coefficients, double T)
{
...
for (size_t i=0; i<PairsMM.size()/2; i++ )
{
        if (2*i != PairsMM.size())
        {
...

                {
                	for(size_t j=0; j<4; j++)
                	{
                		coefficients.push_back(C_MMomega22[4*i+j]);
                	}
                 }
        }
...
void MMCollisionInt::getCoefficients_bstar(string s1, string s2, vector<double> &coefficients, double T)
{
...
for (size_t i=0; i<PairsMM.size()/2; i++ )
{
        if (2*i != PairsMM.size())
        {
                 if ( ( (PairsMM[2*i] == s1) and (PairsMM[2*i+1] == s2) ) or ( (PairsMM[2*i] == s2) and (PairsMM[2*i+1] == s1) ) )
                {
                        for(size_t j=0; j<4; j++)
                        {
                                coefficients.push_back(C_MMbstar[4*i+j]);
                        }
                 }
        }
...
doublereal MMCollisionInt::omega11_hT(const string* species, double T, int nS)
{

	string s1 = species[0];
	string s2 = species[1];
	vector<double> c;
	doublereal logT[4] = {log(T)*log(T)*log(T), log(T)*log(T), log(T), 1};
        doublereal omega11;

	getCoefficients_omega11(s1, s2, c, T);

	omega11 = exp( (c[0] * logT[0]) + (c[1] * logT[1]) + (c[2] * logT[2]) + (c[3] * logT[3]) );
...
doublereal MMCollisionInt::omega22_hT(const string* species, double T, int nS)
{
	...
	getCoefficients_omega22(s1, s2, c, T);
   
        omega22 = exp( (c[0] * logT[0]) + (c[1] * logT[1]) + (c[2] * logT[2]) + (c[3] * logT[3]) );
...
doublereal MMCollisionInt::bstar_hT(const string* species, double T, int nS)
{
        ...
        getCoefficients_bstar(s1, s2, c, T);

        bstar = exp( (c[0] * logT[0]) + (c[1] * logT[1]) + (c[2] * logT[2]) + (c[3] * logT[3]) );
...
doublereal MMCollisionInt::bstar_charged(const string species1, const string species2, int charge1, int charge2, double T, double Te, double Xe, double P)
{
...
doublereal MMCollisionInt::omega11_charged(const string species1, const string species2, int charge1, int charge2, double T, double Te, double Xe, double P)
{
...
doublereal MMCollisionInt::omega22_charged(const string species1, const string species2, int charge1, int charge2, double T, double Te, double Xe, double P)
{
...
doublereal MMCollisionInt::omega12_charged(const string species1, const string species2, int charge1, int charge2, double T, double Te, double Xe, double P)
{
	...
	else if (    ( (species1 == "E") and ( charge == 0 ) ) or ( ( charge == 0 ) and (species2 == "E") ) )
                {

			doublereal omega11_el;
        		const int nS = 2;
        		string species[nS] = {species1, species2};
        		omega11_el = omega11_hT(species, T, nS) * pow(10, -20);

                        num = 1;
                        omega12 = omega11_el * num;

                }


	// omega12 set equal to omega11 for electron-neutral; missing data for repulsive interaction electron-ion;
	else  
		{
			num = 1;
                        omega12 = 1;
...
doublereal MMCollisionInt::omega13_charged(const string species1, const string species2, int charge1, int charge2, double T, double Te, double Xe, double P)
{
	...
	vector<double> c;
	getCoefficients_bstar(species1, species2, c, T);
	charge = charge1*charge2;
	...
	if (    ( (species1 == "E") and ( charge < 0 ) ) or ( ( charge < 0 ) and (species2 == "E") ) )
			...
			doublereal omega11_el;
        		...
        		omega11_el = omega11_hT(species, T, nS) * pow(10, -20);
                        ...
                        num = 1*pow(10, -20) * exp( (c[0]*lnTste*lnTste*lnTste) + (c[1]*lnTste*lnTste) + (c[2]*lnTste) + c[3] );
                        omega13 = 1.25*omega12 - 0.25*omega11_el*num;
                }
...
doublereal MMCollisionInt::omega14_charged(const string species1, const string species2, int charge1, int charge2, double T, double Te, double Xe, double P)
{
	...
	// missing data for repulsive interaction electron-ion
        else
                {
                        omega14 = omega13;
                }
...
doublereal MMCollisionInt::omega15_charged(const string species1, const string species2, int charge1, int charge2, double T, double Te, double Xe, double P)
{
...
doublereal MMCollisionInt::omega23_charged(const string species1, const string species2, int charge1, int charge2, double T, double Te, double Xe, double P)
{
...
doublereal MMCollisionInt::omega24_charged(const string species1, const string species2, int charge1, int charge2, double T, double Te, double Xe, double P)
{
