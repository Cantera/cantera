#!/usr/bin/env julia generate_bindings.jl
...
function parse_header(src::AbstractString)
    ...
    funcs = CFunc[]
    for m in eachmatch(
            r"\b(int32_t|int64_t|double|void)\s+([A-Za-z_]\w*)\s*\(([^;{]*)\)\s*;"s,
            src)
...
function main(args)
    ...
    open(joinpath(out_dir, "_manifest.jl"), "w") do io
        println(io,
            "# Generated manifest of low-level binding files (see " *
    ...
    println("Generated $total wrappers across $(length(available)) headers into " *
            "$out_dir")
end
