...
class Chebyshev(KineticsModel):
    ...
    :param kunits:
        Units string for the rate constant
    :param Tmin:
    ...
    def __init__(self, coeffs, kunits='', *, Tmin, Tmax, Pmin, Pmax, **kwargs):
        KineticsModel.__init__(self, **kwargs)
        self.Tmin = Tmin
        self.Tmax = Tmax
        self.Pmin = Pmin
...
class ThreeBody(KineticsModel):
    ...
    def reduce(self, output):
        ...
        output['rate'] = self.high_rate.as_yaml()
...
class Falloff(ThreeBody):
    ...
    def reduce(self, output):
        output['type'] = 'falloff'
        output['low-rate'] = self.low_rate.as_yaml()
        output['high-rate'] = self.high_rate.as_yaml()
        ...
        for i,line in enumerate(lines):
            ...
            if speciesName in self.species_dict:
                ...
                if self.species_dict[speciesName].transport is None:
                    self.species_dict[speciesName].transport = TransportData(*data, comment)
                else:
